...
def build_html(metrics: dict, report_filename: str) -> str:
    ...
    # ── Region cards ──────────────────────────────────────────────────────────
    region_color_map = {"South": "#2E7D32", "North": "#0D7C7C", "West": "#E65100"}
    region_cards = ""
    for region in ["South", "North", "West"]:
        rm    = metrics["regions"].get(region, {})
        rate  = rm.get("engagement_rate", 0)
        clr   = region_color_map.get(region, "#455A64")
        rc    = rate_color(rate)
        total = rm.get("total_tasks", 1) or 1
        pos_w = round(rm.get("positive", 0) / total * 100)
        neu_w = round(rm.get("neutral",  0) / total * 100)
        neg_w = 100 - pos_w - neu_w
        ttc   = fmt_hours(rm.get("avg_time_to_close_hours", 0))
        region_cards += f"""
        <div class="region-card" style="border-top:4px solid {clr};">
          ...
          <div class="region-sub">{rm.get('total_tasks',0)} tasks &nbsp;|&nbsp; Avg close: {ttc}</div>
          <div class="sentiment-bar">
            <div class="bar-pos" style="width:{pos_w}%"></div>
            <div class="bar-neu" style="width:{neu_w}%"></div>
            <div class="bar-neg" style="width:{neg_w}%"></div>
          </div>
    ...
    for t in open_tasks:
        ...
        <tr class="{row_class}">
          ...
          <td><span class="badge" style="background:{badge_clr}">{region or '?'}</span></td>
          <td>{t.get('source','')}</td>
          <td style="color:{score_clr};font-weight:bold;">{score:.1f}</td>
          <td style="color:{day_clr};font-weight:bold;">{days}d</td>
...
<head>
  ...
  <title>WMCHealth ORM CTL Report – {month_label}</title>
  <style>
    *{{box-sizing:border-box;margin:0;padding:0;}}
    ...
    .dl-btn{{background:#0D7C7C;color:white;padding:10px 20px;border-radius:6px;
             text-decoration:none;font-weight:600;font-size:13px;margin-left:20px;white-space:nowrap;}}
    .dl-btn:hover{{background:#0a6060;}}

    .container{{max-width:1200px;margin:0 auto;padding:28px 24px;}}

    .kpi-grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:28px;}}
    .kpi-card{{background:white;border-radius:8px;padding:20px;box-shadow:0 2px 6px rgba(0,0,0,.07);border-top:4px solid #ddd;}}
    .kpi-val{{font-size:36px;font-weight:800;margin:8px 0 4px;}}
    .kpi-label{{font-size:11px;font-weight:700;color:#607D8B;text-transform:uppercase;letter-spacing:.5px;}}
    ...
    .region-card{{background:white;border-radius:8px;padding:18px;box-shadow:0 2px 6px rgba(0,0,0,.07);}}
    ...
    .table-wrap{{background:white;border-radius:8px;box-shadow:0 2px 6px rgba(0,0,0,.07);overflow:hidden;margin-bottom:28px;}}
    ...
    .archive{{background:white;border-radius:8px;padding:20px;box-shadow:0 2px 6px rgba(0,0,0,.07);}}
...
<div class="container">
  ...
  <div class="section-title">⚠ Open &amp; Critical Tasks (Top {task_count} by Urgency)</div>
...
<div class="footer">
  WMCHealth ORM CTL Dashboard &nbsp;·&nbsp; Powered by Press Ganey API &nbsp;·&nbsp;
